...
# The old `google.generativeai` SDK is deprecated and the model id this
# service used to call ("gemini-2.0-flash-exp") has been retired by
# Google -- together those two things were the reason every /chat request
# was failing. We now use the current, actively-maintained `google-genai`
# SDK (`pip install google-genai`, see requirements.txt).
...
class GeminiService:
    ...
    def generate_response(
        ...
        try:
            ...
            if not answer:
                # The API can return an empty candidate (e.g. blocked by a
                # safety filter) without raising -- surface that clearly
                # instead of silently returning "".
                raise GeminiAPIError(
                    "Gemini returned an empty response (it may have been "
                    "blocked by a safety filter). Try rephrasing the question."
                )
