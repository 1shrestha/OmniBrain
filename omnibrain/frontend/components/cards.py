...
def page_hero(icon: str, title: str, subtitle: str = "", badge_text: str | None = None) -> None:
    """
    Bold animated-gradient banner used at the top of every page — the
    'landing page' look, instead of the old flat page_header title.
    """
    badge_html = f'<div class="ob-hero-badge">{badge_text}</div>' if badge_text else ""
    st.markdown(
        f"""
        <div class="ob-hero">
            <div class="ob-hero-title">{icon} {title}</div>
            <div class="ob-hero-sub">{subtitle}</div>
            {badge_html}
        </div>
        """,
        unsafe_allow_html=True,
