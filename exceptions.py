...
class InvalidURLError(OmniBrainError):
    """Raised when a website URL fails validation."""

    def __init__(self, url: str) -> None:
        super().__init__(
            message=f"'{url}' is not a valid, reachable URL",
            status_code=400,
            details={"url": url},
        )


class WebsiteFetchError(OmniBrainError):
    """Raised when a website cannot be scraped."""

    def __init__(self, url: str, reason: str) -> None:
        super().__init__(
            message=f"Failed to analyze '{url}': {reason}",
            status_code=502,
            details={"url": url, "reason": reason},
        )


class ProjectNotFoundError(OmniBrainError):
...
class SimulationError(OmniBrainError):
    """Raised when simulation start/stop/state fails."""
