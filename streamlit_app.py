...
DEFAULT_API_URL = os.getenv("OMNIBRAIN_API_URL", "http://localhost:8000")
API_PREFIX = "/api/v1"
...
class OmniBrainAPI:
    ...
    def ingest_url(self, url: str) -> Optional[dict[str, Any]]:
        try:
            resp = self._request(
                "POST",
                "/ingest-url",
...
def render_upload_tab() -> None:
    """Render the document upload and URL ingestion section."""
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown("<h3>📤 Upload Documents or Website</h3>", unsafe_allow_html=True)

    uploaded_file = st.file_uploader(
        "Choose a PDF or text file to upload and index",
        type=["pdf", "txt", "md", "csv", "json", "html", "htm"],
        label_visibility="collapsed",
        help="Upload a document for AI-powered Q&A",
    )
    ...
    st.markdown("---")
    with st.expander("🔗 Ingest from website URL", expanded=False):
        ...
        if st.button("🌐 Ingest URL", use_container_width=True):
            if not st.session_state.connected or not st.session_state.api:
                ...
                with st.spinner("Fetching and indexing website content..."):
                    result = st.session_state.api.ingest_url(website_url)
                    if result and "error" not in result:
                        st.session_state.upload_result = result
                        refresh_documents()
                        st.rerun()
                    else:
                        st.error(f"❌ URL ingestion failed: {result.get('error', 'Unknown error')}")
